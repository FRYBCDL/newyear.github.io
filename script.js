...
//修改文字烟花簇的颜色和时间
fireworks.push(new Firework(width / 2, height, 0, -9.5, 1, "gold", true));
setInterval(function () {
  fireworks.push(new Firework(width / 2, height, 0, -9.5, 1, "gold", true));
}, 5000);
...
function TitleParticle(x, y, vx, vy) {
  ...
  this.render = function () {
    circle(this.x, this.y, this.radius, "rgba(252,228,15, " + (this.health / this.maxHealth) + ")");
  }
}

//播放烟花，修改彩色烟花簇的颜色和尺寸
function Firework(x, y, vx, vy, radius = 1, color = "gold", title = false) {
  this.x = x;
  this.y = y;
  this.vx = vx;
  this.vy = vy;
